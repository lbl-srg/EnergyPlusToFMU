...
///
/// \author Thierry S. Nouidui, Wangda Zuo
...
//--- Given an attribute name, return attribute value, or NULL.
//
//   Seek attribute {a} among names in name-value pair array {attNamesVals}.
// Assume {nn} is even, and gives count of strings in {attNamesVals}.
//
static const char* getAttValue(const char** attNamesVals, const int nn, const Att a)
  {
  const char *seekAttName = attNames[(int)a];
  int idx;
  //
  // Inspect attribute names (even-indices) for a match to {seekAttName}.
  for( idx=0; idx<nn; idx+=2 )
    {
    if( seekAttName == attNamesVals[idx] )
      {
      // Return corresponding attribute value (at odd index).
      return( attNamesVals[idx+1] );
      }
    }
  //
  return( NULL );
...
const char* getString(void* element, Att a){
    // hoho should make {element} a {const Element * const}, not a {void *}.
    const Element *const e = (const Element *)element;
    return( getAttValue(e->attributes, e->n, a) );
}
...
double getDouble(void* element, Att a, ValueStatus* vs){
    ...
    double d = 0;
    const Element *const e = (const Element *)element;
    const char* value = getAttValue(e->attributes, e->n, a);
    ...
    if (!value) {
        *vs = valueMissing;
        switch (a) {
            ...
            default: return -1;  // hoho Returning {-1} rather than an {Enu}, and can't coerce {-1} into an {Enu} sensibly.
        }
    }
    id = checkEnumValue(value);
    if (id==-1) *vs = valueIllegal;
    return( (Enu)id );  // hoho Note possible to try to coerce {-1} into {Enu}; undefined.
}
...
const char* getModelIdentifier(ModelDescription* md) {
    const char* modelId = getAttValue(md->attributes, md->n, att_modelIdentifier);
...
const char* getName(void* element) {
    // hoho should make {element} a {const Element * const}, not a {void *}.
    const Element *const e = (const Element *)element;
    const char* name = getAttValue(e->attributes, e->n, att_name);
...
static int sameBaseType(Elm t1, Elm t2){
    return t1==t2 ||
...
const char* getString2(ModelDescription* md, void* tp, Att a) {
    // hoho should make {tp} a {const Type * const}, not a {void *}.
    // hoho Note assumed {tp} is a {Type *}, rather than, say, an {Element *}.
    //   However, in the header file, {void* tp} given as {void* sv}, so it might even be a {ScalarVariable *}.
    //   Looking at code for fcn getVariableAttributeString() strengthens the impression that
    //   what is here called {tp} is actually meant to be a {ScalarVariable *} like {sv} in that fcn.
    Type* type;
    const Type *const tt = (const Type *)tp;
    const char* value = getAttValue(tt->attributes, tt->n, a);
    if (value) return value; // found
    // search declared type, if any
    type = getDeclaredType(md, getAttValue(tt->attributes, tt->n, att_declaredType));
    return type ? getAttValue(type->typeSpec->attributes, type->typeSpec->n, a) : NULL;
}
*/
...
const char * getDescription(ModelDescription* md, ScalarVariable* sv) {
    const char* value = getAttValue(sv->attributes, sv->n, att_description);
    Type* type;
    if (value) return value; // found
    // search declared type, if any
    type = getDeclaredType(md, getAttValue(sv->typeSpec->attributes, sv->typeSpec->n, att_declaredType));
    return type ? getAttValue(type->attributes, type->n, att_description) : NULL;
}
...
const char * getVariableAttributeString(ModelDescription* md,
    ...
    if (!sv) return NULL;
    value = getAttValue(sv->typeSpec->attributes, sv->typeSpec->n, a);
    if (value) return value; // found
    // search declared type, if any
    tp = getDeclaredType(md, getAttValue(sv->typeSpec->attributes, sv->typeSpec->n, att_declaredType));
    return tp ? getAttValue(tp->typeSpec->attributes, tp->typeSpec->n, a) : NULL;
}
...
static AstNodeType getAstNodeType(Elm e){
    switch (e) {
...
static int addAttributes(Element* el, const char** attr) {
    int n, a;
    const char** att = NULL;
    for (n=0; attr[n]; n+=2);
    if (n>0) {
...
// Returns NULL to indicate error
static Element* newElement(Elm type, int size, const char** attr) {
...
static void XMLCALL startElement(void *context, const char *elm, const char **attr) {
    ...
    void* e;
    int size;
    elInt = checkElement(elm);
    if (elInt==-1) return; // error
    el = (Elm)elInt;
...
static void XMLCALL endElement(void *context, const char *elm) {
    ...
    int elInt;
    elInt = checkElement(elm);
    if( -1 == elInt ) return; // illegal element error
    el = (Elm)elInt;
    switch(el) {
        case elm_fmiModelDescription:
            {
                 ...
                 child = (ListElement *)checkPop(ANY_TYPE);  // hoho {ANY_TYPE} is {int}; fcn checkPop() wants {Elm}.  Here and elsewhere.
                 if (child->type == elm_CoSimulation_StandAlone || child->type == elm_CoSimulation_Tool) {
                     cs = (CoSimulation*)child;
                     child = (ListElement *)checkPop(ANY_TYPE);
                     if (!child) return;
                 }
        ...
        case elm_Type:
            {
                Type* tp;
                Element* ts = (Element *)checkPop(ANY_TYPE);
                if (!ts) return;
        ...
        case elm_ScalarVariable:
            {
                ScalarVariable* sv;
                Element** list = NULL;
                Element* child = (Element *)checkPop(ANY_TYPE);
                if (!child) return;
                if (child->type==elm_DirectDependency){
                    list = ((ListElement*)child)->list;
                    free(child);
                    child = (Element *)checkPop(ANY_TYPE);
                    if (!child) return;
                }
...
static void XMLCALL handleData(void *context, const XML_Char *s, int len) {
    int n;
    if (skipData) return;
    if (!data) {
...
static void printElement(int indent, void* element){
    int i;
...
static ModelDescription* validate(ModelDescription* md) {
    int error = 0;
    int i;
    if (md->modelVariables)
    for (i=0; md->modelVariables[i]; i++){
        ScalarVariable* sv = (ScalarVariable*)md->modelVariables[i];
        const char* declaredType = getAttValue(sv->typeSpec->attributes, sv->typeSpec->n, att_declaredType);
...
////////////////////////////////////////////////////////////////
fmiInteger getNumOutputVariablesInFMU(ModelDescription *md){
	int i, j;
	void **list;
	int num_output = 0;
		...
		for(j=0; list[j]; j++)
		{
			...
			for(i=0; i<e->n; i+=2)
			{
				if(!strcmp(e->attributes[i], "causality"))
					val = (Enu)checkEnumValue2(e->attributes[i+1]);  
			}
...
////////////////////////////////////////////////////////////////
///  This method is used to get the number of input in the FMU
///
///\param ModelDescription FMU model description file
////////////////////////////////////////////////////////////////
fmiInteger getNumInputVariablesInFMU(ModelDescription *md){
	int i, j;
	void **list;
	int num_input = 0;
