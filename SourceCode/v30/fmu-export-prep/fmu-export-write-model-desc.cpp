...
// Maximum number of inputs of the FMU
...
#define HS_MAX 64
  ...
  //
  xmlOutput_attribute(outStream, -1, "fmiVersion", "3.0");
  xmlOutput_attribute(outStream, 0, "modelName", idfFileBaseName);
  //
  // Find instantiationToken as MD5 checksum of IDF file.
  char hexDigestStr[33];
  digest_md5_fromFile(idfFileName, hexDigestStr);
  xmlOutput_attribute(outStream, 0, "instantiationToken", hexDigestStr);
  //
  ...
  //
  xmlOutput_comment(outStream, 1, "Note instantiationToken is an md5 checksum of the IDF file.");
  ...
  xmlOutput_attribute(outStream, 2, "providesIntermediateUpdate", "false");
  xmlOutput_attribute(outStream, 2, "hasEventMode", "false");
  xmlOutput_attribute(outStream, 2, "mightReturnEarlyFromDoStep", "false");
  xmlOutput_attribute(outStream, 2, "canReturnEarlyAfterIntermediateUpdate", "false");
  xmlOutput_attribute(outStream, 2, "providesDirectionalDerivatives", "false");
  xmlOutput_attribute(outStream, 2, "providesAdjointDerivatives", "false");
  xmlOutput_attribute(outStream, 2, "providesPerElementDependencies", "false");
  xmlOutput_attribute(outStream, 2, "providesEvaluateDiscreteStates", "false");
  ...
  toEP = false;
  valRef = MAX_NUM_INPUTS+1;  // hoho  Should test against unlikely case that have 100001 {toEP} variables.
  ...
  // Reset value reference for outputs
  valRef = MAX_NUM_INPUTS+1;  // hoho  Should test against unlikely case that have 100001 {toEP} variables.
  ...
  for (idx = 0; idx<datCt; ++idx, ++valRef)
  {
	  writeTag_outputVariable(outStream, 3, valRef + idx );
  }
  //
  //-- Close outputs tag.
...
static void writeTag_scalarVariable(std::ostream& outStream, const int indentLevel,
  ...
  const bool toEP, const int idfLineNo, const double initValue)
  {
  // EnergyPlusFMU has only variables from type Float64.
  // Therefore we have replaced the ScalarVariable attribute with Float64
  const char *const scalarVarTagName = "Float64";
  //
  #define HS_MAX 64
  char helpStr[HS_MAX];
  ...
  if (toEP)
  {
      ...
      // indicate that the FMU can't handle multiple set per time instant.
      xmlOutput_attribute(outStream, indentLevel, "canHandleMultipleSetPerTimeInstant", "false");
  }
  xmlOutput_endTag(outStream, -1, NULL);
  #undef HS_MAX
  //
...
static void writeTag_outputVariable(std::ostream & outStream, const int indentLevel,
    int index)
{
    // EnergyPlusFMU has output variables 
    const char* const scalarVarTagName = "Output";
    //
#define HS_MAX 64
    char helpStr[HS_MAX];
